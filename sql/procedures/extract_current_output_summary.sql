CREATE DEFINER=`root`@`localhost` PROCEDURE `extract_current_output_summary`(IN area_code varchar(10), IN tier1adjustment decimal(10,2), IN tier2adjustment decimal(10,2), IN output_dir varchar(200))
BEGIN
...
SET @output_file := CONCAT( "current_overall_results_", @area_code, DATE_FORMAT( NOW(), "%Y-%m-%d_%k%i%s" ) );
SET @csv_enclose_char         := '"';
SET @csv_field_terminate_char := ',';
SET @csv_line_terminate_char  := '\n';
Drop table if exists current_output_tmp;
set @area_id = (select id from area where ar_code = @area_code);

CREATE Temporary Table current_output_tmp
...
from (select area_id, status, tier from output_timeseries where tier = 1 and stock_id <> 8140 and area_id = @area_id union all select area_id, status, tier from output where area_id = @area_id and stock_id <> 8140) as tmp_out;
...
SET @final := CONCAT( "SELECT * FROM (",
    "(", @header, ")",
     "UNION ALL",
    "(", "select area_code, underfished, fullyfished, overfished, totals from current_output_tmp ", ")",
