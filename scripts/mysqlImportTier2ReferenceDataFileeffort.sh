...
# set defaults
  ...
  user="mysql"
  schema="tafp"
  sqlDir="C://Users/Nicole/Desktop/fish/rishi raw data/area 31/Data_files_Area_31_3/effort/"
  sqlFile="Yellowtail_snapper_Cuba_Effort.csv"
  area="31"
  terminated=','
  enclosed='"'
...
LOAD DATA 
  ...
  INTO TABLE tmp_capture
       ...
       IGNORE 1 LINES
       (@stockid,@scientificname,@commonname,@year,@catch,@best_effort,@stocklong)
       SET cap_stock = @scientificname,cap_common=@commonname, cap_year = @year, catch = @catch,
       best_effort = @best_effort, stocklong = @stocklong;
...
SELECT distinct trim(replace(replace(replace(replace(stocklong,'Area31',''),'_',' '),cap_common,''),'total landings','')),stocklong
FROM tmp_capture tc
left join location loc on tc.stocklong = loc.location_code
where loc.location_id is null;

INSERT INTO capture(area_id, location_id, stock_id, stock_update_id, year, biomass, unit, index_value, effort, reference_id, tier)
SELECT ar.id, loc.location_id, st.stock_id,st.stock_update_id, tc.cap_year, tc.catch, @measure, tc.best_index, tc.best_effort, 454, 2
 FROM tmp_capture tc
