...
# set defaults
  ...
  user="mysql"
  schema="tafp"
  sqlDir="C://Users/Nicole/Desktop/fish/rishi raw data/area 31/Data_files_Area_31_3/index/"
  sqlFile="Total_snooks_Mexico_Index.csv"
  area="31"
  terminated=','
  enclosed='"'
...
LOAD DATA 
  ...
  INTO TABLE tmp_capture
       ...
       IGNORE 1 LINES
       (@stockid,@scientificname,@commonname,@year,@catch,@best_index,@stocklong)
       SET cap_stock = @scientificname,cap_common=@commonname, cap_year = @year, catch = @catch,
       best_index = @best_index, stocklong = @stocklong;
